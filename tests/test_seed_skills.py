...
def test_the_google_skill_sends_availability_to_a_busy_time_read() -> None:
    ...
    shared calendar. A conflict check pairs commitments across calendars and
    skips same-calendar pairs, and runs per account, so an empty result
    settles neither availability nor double-booking."""
    ...
    assert "skips two that overlap on the SAME one" in body
    flat = body.replace("\n   ", " ")
    assert "two commitments on different accounts are never compared" in flat
    assert "not that the owner is free, and not even that nothing is double-booked" in flat
